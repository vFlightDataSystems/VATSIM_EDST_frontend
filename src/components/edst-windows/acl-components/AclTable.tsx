...
export function AclTable() {
    ...
    return (<div className="acl-body no-select">
        ...
        <div className="scroll-container">
            {spa_entry_list?.map(([i, e]: [string, EdstEntry]) =>
                <AclRow
            ...
            {Object.entries(entry_list?.filter((entry: EdstEntry) => (!(typeof (entry.spa) === 'number') && ((entry.acl_status > -1) || !manual_posting)))
                ?.sort(sortFunc))?.map(([i, e]: [string, EdstEntry]) =>
                <AclRow
            ...
            {manual_posting && Object.entries(entry_list?.filter((entry: EdstEntry) => (!(typeof (entry.spa) === 'number') && cid_list.has(entry.cid) && entry.acl_status === -1)))
                ?.map(([i, entry]: [string, EdstEntry]) =>
                    <AclRow
                        key={`acl-table-row-no-ack-${entry.cid}`}
                        entry={entry}
                        bottom_border={Number(i) % 3 === 2}
                        any_holding={any_holding}
                        hidden={hidden}
