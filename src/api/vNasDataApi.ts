...
export const login = async (code: string, redirectUrl: string) => {
  return fetch(
    `${import.meta.env.REACT_APP_NAS_SERVER_URL}/api/auth/login?code=${code}&redirectUrl=${redirectUrl}&clientId=${
...
export const refreshToken = async (vatsimToken: string) => {
  return fetch(`${import.meta.env.REACT_APP_NAS_SERVER_URL}/api/auth/refresh?vatsimToken=${vatsimToken}`).then(r =>
